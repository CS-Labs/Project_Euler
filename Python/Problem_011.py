...
def problemEleven():
    nums = n.split(' ') # Split the string into a list of the numbers.
    high, right, left, up, down, NE, NW, SE, SW = 0, 0, 0, 0, 0, 0, 0, 0, 0
    # Create empty 20x20 matrix.
    m = [[0 for x in range(20)] for y in range(20)]
    # Fill the matrix with the values.
    for i in range(20):
        for j in range(20):
            m [i][j] = int(nums[(i * 20) + j])
    # Iterate through each element in the matrix and check the product
    # of adjacent numbers in the eight possible directions.
    for i in range(20):
        for j in range(20):
